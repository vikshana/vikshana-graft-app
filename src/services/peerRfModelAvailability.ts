...
export function formatPeerRfUnavailableExplanation(opts: {
    ...
    const lines = [
        ...
        ``,
        `**What is required before Graft can create this panel:**`,
        `1. Add machine \`${machineId}\` (with Module ${moduleNumber} target + peer module features) to the exporter’s \`peer_rf_config.json\`.`,
        `2. Ensure \`ENABLE_PEER_RF=1\` and \`ENABLE_PEER_RF_BACKFILL=1\` on the data bridge.`,
        `3. Restart / re-run peer-RF backfill so Influx receives \`upper\` / \`lower\` / \`expected\` for \`${field}\`.`,
        `4. Re-run this create prompt after bands exist — or, if Graft peer-RF control is configured, reply:`,
        `   **Enroll peer-RF for ${machineId} and create the panel**`,
        ``,
        `Until then Graft will **not** create placeholder band queries (they would only show Module ${moduleNumber} Actual).`,
        ``,
