...
/**
 * Attempts to parse DataFindings from a specialist's JSON response content.
 * Returns undefined if the content is not valid JSON or doesn't match the schema.
 */
function parseDataFindings(content: string, categories: string[]): DataFindings | undefined {
    const hasLoki = categories.includes('loki');
    const hasPrometheus = categories.includes('prometheus');
...
If a tool returns an error, explain what failed briefly and continue if possible.${dataOutputNote}`;
    ...
    try {
        ...
        const dataFindings = parseDataFindings(fullContent, step.toolCategories);
