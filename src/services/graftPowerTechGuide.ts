...
export function buildPowerTechOperatorGuide(): string {
    return [
        ...
        '- **Instrumentation metrics (Keysight)**: live data is Prometheus (`Pressure1_psi{machine="2505-200033"}`, etc.) — not Influx `keysight_machine`. For “create N panels for every metric”, call `list_prometheus_label_values` on `__name__` with `{machine="MACHINE_ID"}` before inventing panels.',
