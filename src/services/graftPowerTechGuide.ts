...
export function buildPowerTechOperatorGuide(): string {
    return [
        ...
        '- **Instrumentation metrics (Keysight)**: live data is Prometheus `machine_metrics{machine="2505-200033", field="Pressure1_psi"}` (175+ fields) — not standalone `Pressure1_psi{...}` or Influx `keysight_machine`. For “create N panels for every metric”, call `list_prometheus_label_values` on label **`field`** with `{__name__="machine_metrics", machine="MACHINE_ID"}` before inventing panels.',
